<main id="main" class="form-main">
    ...
    <form class="editor-panel" action="{{ $editing ? route('link.update', $link) : route('link.store') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-field"><label for="category_id">所属分类</label><select name="category_id" id="category_id"><option value="">未分类</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected((string) old('category_id', $editing ? $link->category_id : '') === (string) $category->id)>{{ $category->name }}</option>@endforeach</select><small><a class="back-link" href="{{ route('category.create') }}" target="_blank" rel="noopener">＋ 添加分类（新窗口）</a> · 添加后刷新此页可选</small></div>
